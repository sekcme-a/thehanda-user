...
const Program = () => {
  ...
  const {section, programList} = useData()
  ...
  const handleChange = (event, newValue) => {
    ...
    fetch_program_data(newValue)
  };

  const fetch_program_data = (sectionIndex) => {
    if(!section?.program)
      setList([])
    else {
      const selectedSectionId = section.program[sectionIndex].id
      setList(programList.filter(program => program.sectionsId.includes(selectedSectionId)))
    }
  }
  ...
  return (
    <div className={styles.main_container}>
      ...
      >
        ...
        {
          section?.program?.map((item, index) => {
            return (
